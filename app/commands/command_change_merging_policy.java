 ...
 */
public class command_change_merging_policy implements interface_command {
    @Override
    public boolean is_recognized(model_command command) {
        if ((command.command.equals("change")) && (command.args.size() == 3) &&
                ...
                (command.args.get(1).equals("merging"))) {
            try {
                // try to parse this
    ...
    public String get_command_name() {
        return "CHANGE_MERGING_POLICY";
    }

    @Override
    public String get_command_help() {
        return "change policy merging 30%";
    }
}
