 ...
 */
public class command_change_repo_policy implements interface_command {
    @Override
    public boolean is_recognized(model_command command) {
        if ((command.command.equals("change")) && (command.args.size() == 3) &&
                ...
                (command.args.get(1).equals("repo"))) {
            try {
                // try to parse this
    ...
    public String handle(model_command command, interface_github_webhook hook) {
        ...
        // change the policy!
        BigDecimal new_percent = utils_bigdecimal.from_percent_or_number(command.args.get(2));
        if (new_percent.compareTo(store_conf.get_policy_floor_ownership_required_to_manage_repo()) < 0) {
            return "the ownership to manage the repo must be above " +
                    store_conf.get_policy_floor_ownership_required_to_manage_repo() + "%";
        }

        if (policy == null) {
            policy = new model_repo_policy(hook.get_repo());
            policy = policy.same_but_with_different_policy_to_manage_repo(new_percent);
    ...
    public String get_command_name() {
        return "CHANGE_REPO_POLICY";
    }

    @Override
    public String get_command_help() {
        return "change policy repo 30%";
    }
}
